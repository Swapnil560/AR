<script lang="ts">
  ...
  import { getUsers, loginUser, saveUser } from '../../../services/actions/user.js';
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isLogin) {
        ...
        const existingUsers = await getUsers({ search: `email:${formData.email}` });
        ...
        successMsg = "Verification code sent to your email. Please check your inbox.";
        otpSent = true;
      }
    } catch (err) {
  ...
  const handleVerifyOtp = async () => {
    if (inputOtp === savedOtp) {
      const saveRes = await saveUser(formData);
      if (saveRes.err) {
    ...
    <form on:submit={handleSubmit} class="auth-form">
      {#if otpSent}
        <div class="otp-section">
          ...
          <p class="otp-subtitle">We've sent a verification code to {formData.email}</p>
      ...
      {:else}
        <div class="form-fields">
          ...
          {#if !isLogin}
            <div class="form-group">
              ...
              <div class="input-wrapper">
                <svg class="input-icon" viewBox="0 0 24 24" fill="none">
                  <path d="M22 16.92V19.92C22 20.52 21.52 21 20.92 21C9.4 21 0 11.6 0 0.08C0 -0.52 0.48 -1 1.08 -1H4.08C4.68 -1 5.16 -0.52 5.16 0.08V3.08C5.16 3.68 4.68 4.16 4.08 4.16H2.08C2.08 11.08 7.92 16.92 14.84 16.92V14.92C14.84 14.32 15.32 13.84 15.92 13.84H18.92C19.52 13.84 20 14.32 20 14.92V16.92Z" stroke="currentColor" stroke-width="2"/>
                </svg>
                <input 
                  id="phone"
                  type="tel" 
              ...
              >
                {#if showPassword}
                  <svg viewBox="0 0 24 24" fill="none">
                    <path d="M17.94 17.94C16.2306 19.243 14.1491 19.9649 12 20C5 20 1 12 1 12C2.24389 9.68192 4.02125 7.65663 6.17 6.06L17.94 17.94Z" stroke="currentColor" stroke-width="2"/>
        ...
        <div class="auth-switch">
          ...
          <button 
            type="button" 
            class="switch-btn" 
            on:click={() => { isLogin = !isLogin; error = ""; successMsg = ""; }} 
            disabled={loading}
          >
